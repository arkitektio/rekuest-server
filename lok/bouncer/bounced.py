...
def get_session_app():
    ...
    if SESSION_APP is None:
        ...
        SESSION_APP, created = LokApp.objects.get_or_create(
            client_id="session", name="Session", grant_type=LokGrantType.SESSION.value
        )
...
class Bounced:
    ...
    def bounce(
        self, required_roles=[], required_scopes=[], anonymous=False, only_jwt=False
    ):
        ...
        if self._app.grant_type == LokGrantType.CLIENT_CREDENTIALS.value:
            if not required_scopes.issubset(self.scopeset):
                raise BounceException(
                    f"App has not the required Scopes. Required {required_scopes}"
                )
