    ...
    def mutate(root, info, type, object, userAssignments=[], groupAssignments=[]):
        ...
        for ass in userAssignments:
            ...
            for perm in ass["permissions"]:
                try:
                    assign_perm(
                        perm,
                        get_user_model().objects.get(sub=ass["user"]),
                        instance,
                    )
                except get_user_model().DoesNotExist:
                    raise Exception("One of the users does not exist in rekuest yet. Are you sure you have the right id?")
        ...
        for ass in groupAssignments:
            ...
            for perm in ass["permissions"]:
                assign_perm(
                    perm,
                    Group.objects.get(sub=ass["group"]),
                    instance,
                )
