...
class DelayedBordManager(Manager):
    ...
    def from_dataframe(
        self,
        df: DataFrame,
        fileversion=active_settings.FILE_VERSION,
        apiversion=active_settings.API_VERSION,
        **kwargs
    ):
        ...
        # Actually Saving
        item.unique = uuid4()
        graph = item.parquet.save(
            df, compute=False, fileversion=fileversion, apiversion=apiversion
        )
        item.save()
        return (item, graph)
