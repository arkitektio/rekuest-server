...
def prepare_messages_for_reservation(
    bounced_reserve: BouncedReserveMessage,
) -> List[Tuple[str, MessageModel]]:
    ...
    bounced_reserve.meta.extensions.callback
    ...
    if node and not template:
        ...
        qs = Template.objects.select_related("registry")
    ...
    if template is None:
        raise NoTemplateFoundError(
            f"Did not find a providable Template for this Node {node} with params {params}. No App implementing this Node with this Params was active."
        )

    # Filter based on Assignment Policy and also on activity if deamnded
    provision = template.provisions.exclude(
        status__in=[ProvisionStatus.ENDED, ProvisionStatus.CANCELLED]
    ).first()

    # Here we check if we can assign to the Topic
    if provision is None:
        # PROVIDE PATH
        if "can_provide" not in context.scopes:
            raise DeniedError(
                "Your App does not have the proper permissions set to autoprovide 'can_provide' to your scopes"
            )
        if params.auto_provide != True:
            raise ReserveError(
                "You didn't specify auto_provide in the provide params and this configuration is not yet provided!"
            )
        ...
        if provision.creator:
            MyProvisionsEvent.broadcast(
                {"action": "created", "data": provision.id},
                [f"provisions_user_{provision.creator.id}"],
            )
        ProvisionEventSubscription.broadcast(
            {"action": "updated", "data": reservation.id},
            [f"provision_{reservation.reference}"],
        )
        ...
        if template.provider.active:
            ...
            messages.append(
                (f"provision_in_{provision.template.provider.unique}", provide_message)
            )

        else:
            messages += pause_reservation(
                reservation,
                "Waiting for the Agent to come online! So we can provision!",
            )
    ...
    if provision.status == ProvideState.ACTIVE:
        ...
        messages += pause_reservation(
            reservation,
            "Waiting for the Provision to come online! So we can reserve again",
        )
...
def prepare_messages_for_unreservation(bounced_unreserve: BouncedUnreserveMessage):
    ...
    if (
        context.user
        and res.creator
        and context.user != res.creator.email
        and "admin" not in context.roles
    ):
        raise DeniedError(
            "Only the user that created the Reservation or an admin can unreserve his pods"
        )
    ...
    for provision in res.provisions.all():
        if provision.status == ProvisionStatus.ACTIVE:
            ...
            messages.append(
                (channel, bounced_unreserve)
            )  # just forwarding it to the provisino
            requires_unreservation = True
        else:
            log_to_reservation(
                res.reference,
                f"Topic {provision.unique} is not currently active. Unreserving",
            )
            provision.reservations.remove(res)

    if not requires_unreservation:
        messages += cancel_reservation(
            res, f"No active Topics need cancellation. We can shutdown", reconnect=False
        )
...
def prepare_messages_for_unassignment(bounced_unassign: BouncedUnassignMessage):
    ...
    if context.user != ass.creator.email and "admin" not in context.roles:
        raise DeniedError(
            "Only the user that created the Assignment or an admin can unassign"
        )
    messages = []
    ...
    if ass.reservation.status in [ReservationStatus.ENDED, ReservationStatus.CANCELLED]:
        ...
        messages.append(
            (
                callback,
                UnassignDoneMessage(
                    data={"assignation": ass.reference},
...
def prepare_messages_for_assignment(bounced_assign: BouncedAssignMessage):
    ...
    if context.user != res.creator.email and "admin" not in context.roles:
        raise DeniedError(
            "Only the user that created the Reservation or an admin can assign to it"
        )
...
def prepare_messages_for_unprovision(bounced_unprovide: BouncedUnprovideMessage):
    ...
    bounced_unprovide.meta.reference
    context = bounced_unprovide.meta.context
    prov = Provision.objects.get(reference=provision_reference)

    if context.user != prov.creator.email and "admin" not in context.roles:
        raise DeniedError(
            "Only the user that created the Reservation or an admin can unreserve his pods"
        )
    ...
    if prov.status == ProvisionStatus.ACTIVE:
        messages.append(
            (f"provision_in_{prov.template.provider.unique}", bounced_unprovide)
        )

    else:
        messages += cancel_provision(
            prov,
            "Cancelled on a system level because provision was already not active (provider will not receive it as an initial status",
        )
...
class ReserverRabbit(BaseHare):
    ...
    async def log_to_provision(
        self, reference, message, level=LogLevel.INFO, logCallback=None
    ):
        if logCallback is not None:
            reserve_progress = ProvideLogMessage(
                data={"level": level, "message": message}, meta={"reference": reference}
            )
    ...
    async def on_bounced_reserve_in(
        ...
        try:
            ...
            try:
                messages = await sync_to_async(prepare_messages_for_reservation)(
                    bounced_reserve
                )
    ...
    async def on_bounced_unreserve_in(
        ...
        try:
            ...
            try:
                messages = await sync_to_async(prepare_messages_for_unreservation)(
                    bounced_unreserve
                )
    ...
    async def on_bounced_unprovide_in(
        ...
        try:
            ...
            try:
                messages = await sync_to_async(prepare_messages_for_unprovision)(
                    bounced_unreserve
                )
    ...
    async def on_bounced_assign_in(
        ...
        try:
            messages = await sync_to_async(prepare_messages_for_assignment)(
                bounced_assign
            )
            for channel, message in messages:
            ...
            await sync_to_async(log_to_assignation)(
                reference, str(e), level=LogLevel.CRITICAL
            )
            await sync_to_async(set_assignation_status)(
                reference, AssignationStatus.CRITICAL.value
            )
            if callback:
    ...
    async def on_bounced_unassign_in(
        ...
        try:
            messages = await sync_to_async(prepare_messages_for_unassignment)(
                bounced_unassign
            )
