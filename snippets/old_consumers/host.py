...
from delt.messages.types import (
    BOUNCED_FORWARDED_ASSIGN,
    BOUNCED_FORWARDED_RESERVE,
    RESERVE_DONE,
    UNRESERVE_DONE,
)
import json
...
def activateProvision(
    provision_reference,
) -> Tuple[str, List[str], List[Tuple[str, MessageModel]]]:
    ...
    for res in provision.reservations.all():
        ...
        messages.append(
            log_to_reservation(
                res.reference,
                f"Listening now to {provision.unique}",
                level=LogLevel.INFO,
            )
        )
...
def cancelProvision(
    provision_reference,
) -> Tuple[List[str], List[Tuple[str, MessageModel]]]:
    """Activatets the provision and sets the created topic
    to active as well as creating signal for every connecting Reservation that this Topic
    is now active. (This Reservations should have been previously waiting)
    ...
    for res in provision.reservations.all():
        ...
        messages.append(
            log_to_reservation(
                res.reference,
                f"Provision cancelled shutdown {provision.unique}",
                level=LogLevel.WARN,
            )
        )
...
def killProvision(
    ...
    for res in provision.reservations.all():
        ...
        messages.append(
            log_to_reservation(
                res.reference,
                f"Lost Connection to {provision.unique}",
                level=LogLevel.WARN,
            )
        )
...
def addReservationToProvision(
    provision_reference, reservation_reference
) -> Tuple[str, str, MessageModel]:
    """Activatets the provision and sets the created topic
    to active as well as creating signal for every connecting Reservation that this Topic
    is now active. (This Reservations should have been previously waiting)
...
def deleteReservationFromProvision(
    provision_reference, reservation_reference
) -> Tuple[str, str, MessageModel]:
    """Activatets the provision and sets the created topic
    to active as well as creating signal for every connecting Reservation that this Topic
    ...
    messages.append(
        log_to_reservation(
            reservation.reference,
            f"Stopped Listening to {provision.unique}",
            level=LogLevel.INFO,
        )
    )
    ...
    if reservation.provisions.count() == 0:
        messages += transition_reservation(
            reservation.reference, ReservationStatus.CANCELLED
        )
...
class HostConsumer(BaseConsumer):  # TODO: Seperate that bitch
    ...
    async def on_reservation_related(
        ...
        if isinstance(message, BouncedReserveMessage):
            ...
            channel, messages = await sync_to_async(addReservationToProvision)(
                provision_reference, reservation_reference
            )
            # Set up connectiongs
            assert (
            ...
            messages = await sync_to_async(deleteReservationFromProvision)(
                provision_reference, reservation_reference
            )
    ...
    async def disconnect(self, close_code):
        try:
            ...
            await asyncio.gather(
                *[
                    sync_to_async(killProvision)(prov)
                    for prov, queues in self.provision_link_map.items()
                ]
            )
    ...
    async def on_assign_critical(self, assign_critical: AssignCriticalMessage):
        if assign_critical.meta.extensions.persist:
            await sync_to_async(log_to_assignation)(
                assign_critical.meta.reference,
                assign_critical.data.type + " : " + assign_critical.data.message,
                level=LogLevel.CRITICAL,
            )
            await sync_to_async(set_assignation_status)(
                assign_critical.meta.reference, AssignationStatus.CRITICAL
            )
    ...
    async def on_assign_yields(self, assign_yield: AssignYieldsMessage):
        if assign_yield.meta.extensions.persist:
            await sync_to_async(log_to_assignation)(
                assign_yield.meta.reference,
                f"Yielded {assign_yield.data.returns}",
                level=LogLevel.INFO,
            )
    ...
    async def on_assign_return(self, assign_return: AssignReturnMessage):
        if assign_return.meta.extensions.persist:
            await sync_to_async(log_to_assignation)(
                assign_return.meta.reference,
                f"Returned {assign_return.data.returns}",
                level=LogLevel.INFO,
            )
            await sync_to_async(set_assignation_status)(
                assign_return.meta.reference, AssignationStatus.DONE
            )
    ...
    async def on_assign_done(self, assign_done: AssignDoneMessage):
        if assign_done.meta.extensions.persist:
            await sync_to_async(log_to_assignation)(
                assign_done.meta.reference, f"Assignation Done", level=LogLevel.INFO
            )
            await sync_to_async(set_assignation_status)(
                assign_done.meta.reference, AssignationStatus.DONE
            )
    ...
    async def on_provide_transition(self, provide_transition: ProvideTransitionMessage):
        await sync_to_async(log_to_provision)(
            provide_transition.meta.reference,
            f"Transition to {provide_transition.data.state}",
            level=LogLevel.CRITICAL,
        )
        messages = await sync_to_async(transition_provision)(
            provide_transition.meta.reference, provide_transition.data.state
        )
        for channel, message in messages:
        ...
        await self.forward(
            provide_transition, provide_transition.meta.extensions.callback
        )

    async def on_provide_critical(self, provide_critical: ProvideCriticalMessage):
        logger.error(provide_critical)
        await sync_to_async(log_to_provision)(
            provide_critical.meta.reference,
            provide_critical.data.message,
            level=LogLevel.CRITICAL,
        )
        await sync_to_async(set_provision_status)(
            provide_critical.meta.reference, ProvisionStatus.CRITICAL
        )
    ...
    async def on_provide_done(self, provide_done: ProvideDoneMessage):
        ...
        reservation_topic, new_channels, new_messages = await sync_to_async(
            activateProvision
        )(reference)

        reservation_queue = await self.channel.queue_declare(reservation_topic)
        await self.channel.basic_consume(
            reservation_queue.queue, lambda x: self.on_reservation_related(reference, x)
        )

        for channel in new_channels:
            assign_queue = await self.channel.queue_declare(channel)
            await self.channel.basic_consume(
                assign_queue.queue, lambda x: self.on_assign_related(reference, x)
            )
            self.provision_link_map[reference].append(assign_queue)

        if provide_done.meta.extensions.callback:
    ...
    async def on_unprovide_done(self, unprovide_done: UnprovideDoneMessage):
        ...
        if unprovide_done.meta.extensions:
    ...
    async def on_unprovide_log(self, unprovide_log: UnprovideLogMessage):
        ...
        await sync_to_async(log_to_provision)(
            reference, unprovide_log.data.message, unprovide_log.data.level
        )
    ...
    async def on_unprovide_critical(self, unprovide_critical: UnprovideCriticalMessage):
        ...
        await sync_to_async(log_to_provision)(
            reference, unprovide_critical.data.message, level=LogLevel.CRITICAL
        )
    ...
    async def on_unassign_done(self, assign_return: UnassignDoneMessage):
        ...
        if assign_return.meta.extensions.persist:
            await sync_to_async(log_to_assignation)(
                assign_return.data.assignation, "Unassignment Done", level=LogLevel.INFO
            )
            await sync_to_async(set_assignation_status)(
                assign_return.data.assignation, AssignationStatus.CANCELLED
            )
        await self.forward(assign_return, assign_return.meta.extensions.callback)
    ...
    async def on_unassign_critical(self, assign_return: UnassignCriticalMessage):
        try:
            await log_to_assignation(
                assign_return.meta.reference,
                assign_return.data.message,
