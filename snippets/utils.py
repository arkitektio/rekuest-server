...
from delt.messages import ReserveTransitionMessage
from delt.messages.base import MessageDataModel, MessageModel
...
def log_to_reservation(
    reference: str, message: str, level=LogLevel.INFO, persist=True, callback=None
) -> Tuple[str, MessageDataModel]:
    ...
    if persist:
        ...
        ReservationEventSubscription.broadcast(
            ...
            [f"reservation_{reference}"],
        )

    if callback is not None:
        reserve_progress = ReserveLogMessage(
...
def log_to_provision(
    ...
    if persist:
        ...
        ProvisionEventSubscription.send_log(
            [f"provision_{reference}"], message, level=level
        )
...
def transition_reservation_by_reference(
    reference: str, state: ReservationStatus, message: str = "Caused by a Transition"
):
...
def transition_reservation(
    ...
    message: str = "Caused by a Transition",
    ...
    if reservation:
        creator: MyReservationsEvent.broadcast(
            {"action": state.value, "data": reservation.id},
...
def transition_provision_by_reference(
    reference: str, state: ProvideState, message: str = "Transition"
):
    provision = Provision.objects.get(reference=reference)
    return transition_provision(provision, state, message)


def transition_provision(
    reference: str, state: ProvideState, message: str = "Transition"
) -> List[Tuple[str, MessageModel]]:
    provision = Provision.objects.get(reference=reference)
    ...
    if provision.creator:
        MyProvisionsEvent.broadcast(
            {"action": state.value, "data": provision.id},
...
def create_assignation_from_bounced_assign(assign: BouncedAssignMessage):
    ...
    assignation, created = Assignation.objects.update_or_create(
    ...
    MyAssignationsEvent.broadcast(
        {"action": "created", "data": str(assignation.id)},
        [f"assignations_user_{assign.meta.token.user}"],
    )
    AssignationEventSubscription.broadcast(
        {"action": "update", "data": str(assignation.id)},
        [f"assignation_{assign.meta.reference}"],
    )
...
def set_assignation_status(reference: str, status: AssignationStatus):
    ...
    if assignation.creator:
        MyAssignationsEvent.broadcast(
            {"action": status.value, "data": assignation.id},
            [f"assignations_user_{assignation.creator.id}"],
        )


def log_to_assignation(
    reference: str, message: str, level: LogLevel = LogLevel.INFO.value, persist=True
):
